...
echo pass out proto { tcp udp } port { 22 53 80 123 443 }  >> pf.conf
...
#Allow ICMP
echo pass in quick proto { udp tcp icmp } from any to 10.1.1.8 port { 7 } >> pf.conf
echo pass out quick proto { udp tcp icmp } from 10.1.1.8 to any port { 7 } >> pf.conf
...
#Allow ICMP
echo pass in quick proto { udp tcp icmp } from any to 10.1.1.9 port { 7 } >> pf.conf
echo pass out quick proto { udp tcp icmp } from 10.1.1.9 to any port { 7 } >> pf.conf
